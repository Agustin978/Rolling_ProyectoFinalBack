...
import { creaPedido, obtenerPedidos } from "../controllers/pedidos.controllers";

const router = Router();

router.route('/pedidos').get(obtenerPedidos);

router.route('/pedidosnuevo').post([
    check('idProducto')
        .notEmpty()
        .withMessage('El id del producto es necesario'),
    check('nombreUsuario')
        .notEmpty()
        .withMessage('El nombre de usuario es obligatorio para almacenar el pedido')
    ...
    check('nombreProducto')
        .notEmpty()
        .withMessage('El nombre del producto es obligatorio para almacenar el pedido')
    ...
    check('cantidad')
        ...
        .withMessage('La cantidad minima que se puede pedir es 1 y la maxima de 50'),
    check('direccion')
        .notEmpty()
        .withMessage('La direccion a la que se debe realizar el pedido es obligatoria')
        .isLength({min: 5, max:100})
        .withMessage('La direccion tiene un minimo de 5 y un maximo de 100 caracteres'),
    check('estado')
        .notEmpty()
